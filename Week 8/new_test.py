...
class Tests(PythonTestCase):
    ...
    def test_t1_var(self):
        ...
        self.assertEqual(gca().get_lines()[0].get_xdata().tolist(), arange(0,10.51,0.01).tolist() )

    def test_t2_var(self):
        """ The second line contains the correct x values """
        self.assertEqual(gca().get_lines()[1].get_xdata().tolist(), arange(0,10.65,.15).tolist() )

    def test_t3_var(self):
        """ The third line contains the correct x values """
        self.assertEqual(gca().get_lines()[2].get_xdata().tolist(), arange(0,11,0.5).tolist() )

    def test_x1_var(self):
        """ The first line contains the correct y values """
        self.assertEqual(gca().get_lines()[0].get_ydata().tolist(), ((0.5*sin(4*pi*(attempt.t1-0.125))).tolist() ))

    def test_x2_var(self):
        """ The second line contains the correct y values """
        self.assertEqual(gca().get_lines()[1].get_ydata().tolist(), ((0.5*sin(4*pi*(attempt.t2-0.125))+2).tolist() ))
